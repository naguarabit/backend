...
//OBJETIVO: actualiza transaccion, para cancelarla.
...
$sql = "UPDATE transacciones SET"
     ."  status = 'CANC'" //Pago destino agregado
     .", observ_admin = 'Cancelada por operador'" //se marca status PD en curso
     .", date_updated = NOW()"
     .", date_end = NOW()"
  ...
  if($conn->query($sql)) {
    ...
    if ($ok) { //$outp != ''
      ...
      $outp .= ',"mensaje" : "Cancelled Transaction"';
