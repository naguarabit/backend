...
.controller('ctrlTransaccionResumen', ['$scope', '$http', '$routeParams', function($scope, $http, $routeParams) {
  ...
  $scope.capturarParametro = function(){
    ...
    //TODO. SEGURIDAD. validar parametro, que sea numerico entero
...
$scope.formatDateCreated = function (data){
 ...
 /**
  * funcion para cancelar transacción (por el operador o admin)
  * param tipo 10: transaccion cancelada por operador
  **/

  $scope.updateTransaccion_cancel = function($tipo) {

    console.log('transacc- funcion:updateTransaccion_cancel. inicio');
    /*
    var d = {};
    d.id_transaccion = $scope.id_transaccion;
    //TODO. add field usuario_login_check, que seria el user login en data a grabar
    ...
    */
    ...
    if ($tipo == 10){ //actualizar transaccion con PAGO DESTINO 'PD_OK', Y estatus 'FIN'
      update = './../backend/transacc/update_transaccion10.php';
    }
      ...
      if (response.data){
        ...
        if(dataResp.resultado != null)
          ...
          if(dataResp.resultado == 'EXITO'){
            ...
            $scope.msg = "Transacción cancelada con Exito!";
    ...
    console.log('-transacc/resumen.js - function:updateTransaccion_cancel. fin');

  };//updateTransaccion_cancel


  //prepara datos para marcar pago destino como PAGADO
  $scope.cancelTransaction = function() {
    console.info('Se va a cancelar la transaction con id:' + $scope.id_transaccion);
    var d = {};
    d.id_transaccion  = $scope.id_transaccion;
    //TODO. add field usuario_login_operador_pagador
    $scope.dataSave = d;
    $scope.updateTransaccion_cancel(10);
  }  
